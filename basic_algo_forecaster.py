...
def basic_forecast_update(stock_cfg, recent_df, historical_folder="historical_data"):
    ...
    Cleans indices to avoid duplicate index errors.
    Returns config schema consistent with LLM forecaster.
    """
    ...
    print("hsitory data : ", df_hist.shape)

    # --- Use Redis dataframe (recent_df) ---
    df_recent = (
        recent_df.copy()
    ...
    if df_recent.empty:
        ...
        stock_cfg["forecast"] = "basic_algo"
        return stock_cfg

    print("recent data : ", df_recent.shape)

    # --- Merge for computing SR ---
    if not df_hist.empty:
        df_all = pd.concat([df_hist, df_recent], ignore_index=True)
    else:
        df_all = df_recent.copy()

    # --- Compute support, resistance, volume threshold ---
